...
/* Joezie: macro=>func */
template <typename T>
void BoundUpper(T &_x, T _max) { if (_x > (_max)) _x = (_max); }

/* Joezie: macro=>func */
template <typename T1, typename T2, typename T3>
void Bound(T1 &_x, T2 _min, T3 _max) { if (_x > (_max)) _x = (_max); else if (_x < (_min)) _x = (_min); }

/* Joezie: macro=>func */
template <typename T1, typename T2, typename T3>
void BoundInverted(T1 &_x, T2 _min, T3 _max) {           
    if ((_x < (_min)) && (_x > (_max))) {         
...
/* Joezie: macro=>func */
template <typename T1, typename T2, typename T3>
void BoundWrapped(T1 &_x, T2 _min, T3 _max) {
	if ((_max) > (_min)) {        
    if (_x > (_max)) _x = (_max); 
    else if (_x < (_min)) _x = (_min);
  }                   
  else {                            
    if ((_x < (_min)) && (_x > (_max))) {         
    if (abs(_x - (_min)) < abs(_x - (_max)))    
...
void BoundAbs(T1 &_x, T2 _max) { if (_x > (_max)) _x = (_max); else if (_x < -(_max)) _x = -(_max); }

/* Joezie: macro=>func */
template <typename T1, typename T2, typename T3>
T1 Clip(T1 _x, T2 _min, T3 _max) { return (_x) < (_min) ? (_min) : (_x) > (_max) ? (_max) : (_x); }

/* Joezie: macro=>func */
template <typename T1, typename T2>
T1 ClipAbs(T1 _x, T2 _max) { return (_x) < -(_max) ? -(_max) : (_x) > (_max) ? (_max) : (_x); }

/* Joezie: macro=>func */
template <typename T1, typename T2>
T1 DeadBand(T1 &_x, T2 _v) {            
    if (_x > (_v))                    
      _x = _x -(_v);                  
    else if  (_x < -(_v))             
      _x = _x +(_v);                  
